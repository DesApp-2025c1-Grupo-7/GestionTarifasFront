...
const AdicionalesReport = ({ showNotification }) => {
  ...
  // Tu lógica de stats y filtrado se mantiene intacta
  const stats = useMemo(() => {
  ...
  const filteredData = useMemo(() => {
    ...
    filtered.sort((a, b) => {
      switch (sortBy) {
        case 'uso': return b.frecuenciaDeUso - a.frecuenciaDeUso;
        case 'costo': return Number(b.costo) - Number(a.costo);
        case 'alfabetico': return a.descripcion.localeCompare(b.descripcion);
        default: return 0;
      }
    });
    return filtered;
                ...
                <tbody className="bg-[#444240] divide-y divide-gray-700">
                  {filteredData.map(item => (
                    <tr 
                      ...
                      className={`transition-colors ${item.frecuenciaDeUso > 0 ? 'cursor-pointer hover:bg-gray-700' : 'opacity-60'}`}
                    >
