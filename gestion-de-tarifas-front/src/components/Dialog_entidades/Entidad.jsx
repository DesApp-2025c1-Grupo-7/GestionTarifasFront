...
const Entidad = ({ entidad, onClose }) => {
  const handleSubmit = (e) => {
    ...
    if (data.es_peligrosa !== undefined) {
      data.es_peligrosa = formData.get('es_peligrosa') === 'on';
    }
    if (data.es_obligatorio !== undefined) {
      data.es_obligatorio = formData.get('es_obligatorio') === 'on';
    }

    data.id = Date.now();

    const existentes = JSON.parse(localStorage.getItem(entidad)) || [];

    existentes.push(data);
    localStorage.setItem(entidad, JSON.stringify(existentes));
      ...
      {entidad === "Adicional" && (
        <form onSubmit={handleSubmit}>
          ...
          <div style={fieldStyle}>
            ...
            <input type="checkbox" name="es_obligatorio" style={{ marginLeft: "10px" }} />
          </div>
